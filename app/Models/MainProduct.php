...
{
    use BelongsToCompany, HasFactory, HasJsonResourcefulData, InteractsWithMedia;
    ...
    {
        $this->load('products');
        $prices = collect($this->products)->pluck('product_price')->filter()->toArray();
        ...
        $fields = [
            ...
            'min_price' => empty($prices) ? 0 : min($prices),
        ...
        if ($this->product_type == self::VARIATION_PRODUCT && $this->variations) {
