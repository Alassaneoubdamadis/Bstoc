...
class SettingRepository extends BaseRepository
{
    ...
    public function updateSettings($input)
    {
        try {
            ...
            $booleanKeys = ['show_version_on_footer', 'is_currency_right', 'show_logo_in_receipt', 'show_app_name_in_sidebar'];

            foreach ($settingInputArray as $key => $value) {
                if (in_array($key, $booleanKeys, true) && empty($value)) {
                    Setting::updateOrCreate(['key' => $key], ['value' => false]);
                    continue;
                }
            ...
            $input['logo'] = $logoSetting ? $logoSetting->logo : '';
            DB::commit();
