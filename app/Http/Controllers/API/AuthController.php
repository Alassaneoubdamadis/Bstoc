...
class AuthController extends AppBaseController
{
    ...
    public function login(Request $request)
    {
        ...
        $user = User::withoutGlobalScopes()->whereRaw('lower(email) = ?', [strtolower($email)])->first();
        ...
        if (! empty($user->is_platform_admin)) {
            return $this->sendError('Compte plateforme : connectez-vous sur /platform', 403);
        }

        if (empty($user->status)) {
            return $this->sendError('Compte utilisateur désactivé.', 422);
        }

        $company = $user->company_id
            ? \App\Models\Company::withoutGlobalScopes()->with('plan')->find($user->company_id)
            : null;

        $userPermissions = $user->getAllPermissions()->pluck('name')->toArray();
        if ($company) {
            $userPermissions = $company->filterPermissions($userPermissions);
        }
        unset($user->roles);
        ...
        return response()->json([
            'data' => [
                ...
                'subscription_active' => $company ? $company->hasAccess() : false,
            ],
    ...
    public function register(RegisterRequest $request): JsonResponse
    {
        if (! config('app.public_register')) {
            return $this->sendError('Inscription publique désactivée. Contactez B-Stock.', 403);
        }

        $input = $request->all();
        $input['password'] = bcrypt($input['password']);
        $plan = \App\Models\SubscriptionPlan::where('slug', 'starter')->firstOrFail();
        $company = \App\Models\Company::startTrial([
            'name' => $input['first_name'].' '.$input['last_name'],
            'email' => $input['email'],
        ], $plan);

        $input['company_id'] = $company->id;
        $input['is_platform_admin'] = false;
        $user = User::withoutGlobalScopes()->create($input);
        $user->assignRole('admin');
        $company->update(['owner_user_id' => $user->id]);
        app(\App\Services\CompanyProvisioner::class)->seedDefaults($company->id, [
            'name' => $company->name,
            'email' => $input['email'],
        ]);
