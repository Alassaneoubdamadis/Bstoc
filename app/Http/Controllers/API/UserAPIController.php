...
class UserAPIController extends AppBaseController
{
    ...
    {
        if (! company_allows('manage_users')) {
            throw new \Symfony\Component\HttpKernel\Exception\UnprocessableEntityHttpException(
                'Le super-admin n’a pas autorisé ce magasin à créer des utilisateurs.'
            );
        }
        $input = $request->all();
    ...
    public function config(Request $request)
    {
        ...
        $userPermissions = filter_company_permissions($userPermissions);
        ...
        $openRegister = POSRegister::where('user_id', $user->id)
            ->whereNull('closed_at')
            ->exists();
        ...
        return $this->sendResponse([
            ...
            'platform_favicon' => platform_favicon_url(),
